...
## 5. ★ 라벨 자체가 얼마나 믿을 만한가 — 그리고 왜 첫 결론이 틀렸나
...
낮은 일치율을 보고 처음 내린 결론은 **"라벨이 노이즈다"**였다. 그 뒤 조건을 하나씩 통제하며 확인한 기록이다.

**가설 ① 배치 크기 탓이다** — 원래는 40건, 재판정은 25건이었다. 40건으로 맞춰 다시 돌렸다.

| 비교 | 일치율 | κ |
|---|---|---|
| 원래 라벨 ↔ 재판정(배치 25) | 49.0% | 0.169 |
| 원래 라벨 ↔ 재판정(배치 40) | 50.0% | 0.188 |
| **재판정 ↔ 재판정 (25 vs 40)** | **95.0%** | **0.858** |

**기각.** 49 → 50으로 거의 안 변했다. 그리고 같은 날 돌린 두 판정은 서로 95% 일치한다(κ 0.858, *almost perfect*) — **판정 자체는 불안정하지 않다.**

**가설 ② 그 사이 판정 기준이 바뀌었다** — 원래 라벨은 7월 중순에 매겼고 `persona-consultant.md`는 그 뒤 다섯 번 고쳐졌다(뷰티 업종 배제 해제, MarTech 재정렬, 커머스 축 재편…). 7월 버전은 *"1순위 = AX 직행"*, 지금은 *"1순위 = 커머스 데이터"*다. git에서 7월 파일을 꺼내 같은 25건을 다시 판정했다.

| 기준 | 원래 라벨과 일치 | κ | 분포(부적합/검토/권장) |
|---|---|---|---|
| 현재(08-07) | 32.0% | 0.047 | 19 / 6 / 0 |
| **7월(07-17)** | **40.0%** | 0.188 | 19 / 4 / 2 |
| 원래 라벨 | — | — | **5 / 14 / 6** |
| 두 기준끼리 | **92.0%** | 0.792 | — |

**부분 기각.** 8%p만 회복했고, **기준 문서를 통째로 바꿔도 판정은 92% 그대로**다. 주원인이 아니다.

**가설 ③ 프롬프트가 달랐다** — 원래 판정은 `radar.mjs`가 생성한 배치로 돌았다. 그 헤더를 열어 보니 내 재판정 배치에 없던 지시가 있었다.

```
적합도 척도: 9~10 핵심적합 · 7~8 적합 · 5~6 검토 · 1~4 부적합
트랙 비율 5:3:2:0                          ← 판정 분포를 사실상 지정
⚠️ 라벨(대졸요구·경력N년+)은 컷이 아니라 리스크 표시다  ← 보수적 판정을 막는 지시
```

**이게 가장 유력한 설명이다.** 원래 판정은 *"학력·경력 요건으로 자르지 말라"*는 지시를 받은 상태였고 재판정에는 그게 없었다. 그래서 재판정이 일관되게 부적합으로 몰렸다(분포 5/14/6 → 19/4/2).

### 그래서 결론

**라벨이 흔들린 게 아니라 내가 판정 조건을 재현하지 못한 것이다.** 재현성을 잰다면서 정작 원래 조건을 그대로 복원하지 않았다 — 배치 크기만 맞추고 프롬프트 헤더를 빠뜨렸다.

**이 실험의 진짜 산출물은 49%라는 숫자가 아니라 그 과정이다.** 가설을 셋 세웠고 둘이 기각됐고, 첫 결론("라벨이 노이즈")을 두 번 뒤집었다. 그 기록을 지우지 않고 남긴다.

**남는 사실 둘.**

- **모델 성능의 상한은 라벨이 정한다.** macro-F1이 0.55에서 막힌 걸 모델 탓으로 돌리기 전에 라벨을 먼저 의심해야 했고, 이 실험이 그 습관의 값이다
- **판정은 프롬프트에 매우 민감하다.** 기준 문서를 통째로 바꿔도 92% 그대로인데, 배치 헤더 몇 줄이 분포를 5/14/6에서 19/4/2로 바꿨다. **LLM 판정을 라벨로 쓸 거면 기준 문서보다 프롬프트를 버전 관리해야 한다**
